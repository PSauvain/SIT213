...
if (args[i].equals("-snr")) {
											if ((Pattern.matches(
												...
												throw new InformationNonConforme(
														"SNR passé en paramètre non valide ... Fermeture du programme");
											}
										}
...
if (args[i].equals("-help")) {
		...
		case (1):
			if (aleatoire) {
				programme.ApplicationLogiqueParfaite.programme(nombreAEmettre
						+ "", sondes);
			} else {
				programme.ApplicationLogiqueParfaite.programme(chaineAEmettre,
						sondes);
			}
		break;
		case (2):
			if (aleatoire) {
				programme.ApplicationTransmissionAnalogiqueParfaite.programme(
						nombreAEmettre + "", nbEch, amplitudeMin, amplitudeMax,
						forme, sondes);
			} else {
				programme.ApplicationTransmissionAnalogiqueParfaite.programme(
						chaineAEmettre, nbEch, amplitudeMin, amplitudeMax,
						forme, sondes);
			}
		break;
		case (3):
			if (aleatoire) {
				programme.ApplicationTransmissionAnalogiqueBruitee.programme(
						nombreAEmettre + "", nbEch, amplitudeMin, amplitudeMax,
						forme, snr, sondes);
			} else {
				programme.ApplicationTransmissionAnalogiqueBruitee.programme(
